...
function Set-TargetResource
{
    ...
    elseif ($Ensure -eq "Present" -and $currentResource["Ensure"] -eq "Absent")
    {
        ...
        New-Tentacle -name $Name -apiKey $ApiKey -octopusServerUrl $OctopusServerUrl -port $ListenPort -environments $Environments -roles $Roles -DefaultApplicationDirectory $DefaultApplicationDirectory -tentacleDownloadUrl $tentacleDownloadUrl -tentacleDownloadUrl64 $tentacleDownloadUrl64 -tentacleUrl $tentacleIpAddress
...
if($TentacleUrl -eq $null) {
  $ipAddress = Get-MyPublicIPAddress
  $ipAddress = $ipAddress.Trim()
}
else{
  $ipAddress = $tentacleIpAddress
}
